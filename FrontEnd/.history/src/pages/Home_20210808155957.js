...
export const Home = () => {
    const { setTweets, removeAllTweets, winModalOpen, toggleWinModal, loseModalOpen, toggleLoseModal } = useContext(GlobalContext);
    ...
    const generateFourNumbers = (retweetNumber) => {
        ...
        const secondNumber = Math.abs(retweetNumber + Math.floor(Math.random() * 15 + 1) * secondPlusOrMinusRandom)

        const thirdPlusOrMinusRandom = Math.random() < 0.5 ? -1 : 1;
        const thirdNumber = Math.abs(retweetNumber + Math.floor(Math.random() * 10)  * thirdPlusOrMinusRandom)

        const fourthPlusOrMinusRandom = Math.random() < 0.5 ? -1 : 1;
        const fourthNumber = Math.abs(retweetNumber + Math.floor(Math.random() * 5) * fourthPlusOrMinusRandom)
