        ...
        API.getTweets().then(
            res => {
                ...
                keys.forEach((key, index) => {
                    randomNumbersArray.push(140 + Math.floor(Math.random(0, 10)))
                })

                keys.forEach((key, index) => {
                    let individualRow = {user:{}, tweet:{}}
                    individualRow.id = key
                    individualRow.user.name = uniqueNamesGenerator({dictionaries: [adjectives, colors, animals]});
                    individualRow.user.handle = uniqueNamesGenerator({dictionaries: [adjectives, colors, animals]});
                    var pxSize = randomNumbersArray[index]
