...
const TweetCard = (props) => {
    ...
    const generateFourNumbers = (retweetNumber) => {
        ...
        const secondNumber = Math.abs(retweetNumber + Math.floor(Math.random() * 15))
        const thirdNumber = Math.abs(retweetNumber - Math.floor(Math.random() * 5))
        const fourthNumber = Math.abs(retweetNumber - Math.floor(Math.random() * 3))
