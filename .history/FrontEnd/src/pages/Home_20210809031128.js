...
const RANDOM_HYPERPARAM = [30, 50, 80]

export const Home = () => {
    const { removeTweet, guess, setTweets, removeAllTweets, winModalOpen, toggleWinModal, loseModalOpen, toggleLoseModal, currentTweet, 
            addHumanPredictionDifference, addModelPredictionDifference} = useContext(GlobalContext);

    const [modelPredictions, setModelPredictions] = useState(null)
    const [correctRetweetValue, setCorrectRetweetValue] = useState(null)
    const [currentTweetKey, setCurrentTweetKey] = useState(null)
    
    const handleWinCancel = () => {
        removeTweet(currentTweetKey)
        addHumanPredictionDifference(correctRetweetValue - guess)
        addModelPredictionDifference(correctRetweetValue - modelPredictions)
        toggleWinModal()
    };
        ...
        API.getTweets().then(
            res => {
                ...
                keys.forEach((key, index) => {
                    ...
                    individualRow.modelRetweetPredictions = modelRetweetPredictions[key]
                    individualRow.timestamp = timestamp[key]
                    individualRow.time = time[key]
                    ...
                    individualRow.positiveSentiment = positiveSentiment[key]
    ...
    useEffect(() => {
        if(!currentTweet){
            ...
            setModelPredictions(currentTweet.modelRetweetPredictions) 
            setCorrectRetweetValue(currentTweet.tweet.retweets)
            setCurrentTweetKey(currentTweet.id)
    ...
    return (
        <div>
            ...
            <div style = {{padding:10, display:"flex", flexDirection:"column", alignItems:"center"}}>
                ...
                <h3>
                    Our Models Guess: {modelPredictions}
                </h3> 
                <h3>
                    The Correct Answer:  {correctRetweetValue}
                </h3> 
                </div>
                <div >
