...
def get_tweet(row):
     output = ''
     try:
         output = api.get_status(row=row.TweetId).text
     except:
         pass
     return output
     
df = pd.read_fwf(r'D:\Desktop\AI_term_8\twitter\TweetsCOV19_3', names=['TweetId', 'Username','Timestamp','Followers','Friends','Retweets','Favourites','Entities','Sentiment','Mentions','Hashtags','URLS'], error_bad_lines=False)
